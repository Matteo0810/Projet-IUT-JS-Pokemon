...
function detailsPokemon(pokemonId) {
    attacks = Pokemon.all_pokemons[pokemonId].getAttacks();

    const attacksName = attacks.map(attack => attack.name);

    console.log(attacksName);
}
...
for (const pokemon of sortedPokemon) {
    pokemonsElement?.insertAdjacentHTML("afterbegin", `
        <tr onclick="detailsPokemon('${pokemon.pokemonId}')">
